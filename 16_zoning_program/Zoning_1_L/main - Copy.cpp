...
int main(int argc, char* argv[])
{
    ...
    std::cout << "Initialized the MS model" << std::endl;

    BSO::Spatial_Design::MS_Conformal CF(MS, &(BSO::Grammar::grammar_zoning));
    std::cout << "Initialized the CF model" << std::endl;

    CF.make_conformal();
    std::cout << "Made the CF model conformal" << std::endl;

    BSO::Spatial_Design::Zoning::Zoned_Design Zoned(&CF);
    std::cout << "Initialized the zoning model" << std::endl;

    Zoned.make_zoning();
    std::cout << "Finished zoning" << std::endl;
    ...
    BSO::Structural_Design::SD_Building_Results sd_zoned_results = SD_Zoned_Building.get_results();
    BSO::SD_compliance_indexing(sd_zoned_results);
    BSO::Visualisation::visualise(SD_Zoned_Building, 4);

    std::cout << "Finished visualisation" << std::endl;
