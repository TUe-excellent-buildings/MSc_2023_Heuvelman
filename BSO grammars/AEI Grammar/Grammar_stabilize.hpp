...
namespace BSO { namespace Grammar
{
    ...
    void BP_grammar_3(Spatial_Design::MS_Conformal*, Building_Physics::BP_Simulation_Vars*);
		...
		#ifdef ZONING_HPP
		Zoning_Settings zoning_settings = read_zoning_settings("Settings/Zoning_Settings.txt"); // read the zoning settings file
	...
	#ifdef STABILIZE_HPP
		Stabilize_Settings stabilize_settings = read_stabilize_settings("Settings/Stabilize_Settings.txt"); // read the stabilize settings file
	...
	std::cout << "i" << std::endl;
	    ...
	    void SD_grammar_zoning(Spatial_Design::MS_Conformal* CF, Structural_Design::SD_Analysis_Vars* SD)
	    {
			...
			for (unsigned int l = 0; l < Zoned.get_designs().size(); l++)
		    {
				...
				using namespace Structural_Design;
		        BSO::Structural_Design::read_SD_settings("Settings/SD_Settings.txt", SD); // read file with structural design settings
