...
void visualise(BSO::Spatial_Design::MS_Conformal& cf_building, std::string type, unsigned int i, std::vector<int> ZoneIDs, int initial_zones_count)
{
    vpmanager_local.addviewportzoning(new BSO::Visualisation::viewport(new BSO::Visualisation::Zoning_Model(cf_building, type, i, ZoneIDs, initial_zones_count)));
}

void visualise(BSO::Spatial_Design::MS_Conformal& cf_building, unsigned int i, int zone_ID, int initial_zones_count)
{
    vpmanager_local.addviewportzoning(new BSO::Visualisation::viewport(new BSO::Visualisation::Zoning_Model(cf_building, i, zone_ID, initial_zones_count)));
}
...
void setup_pointers() {
    ...
    (*SD_Building).analyse();
    initial_zones = Zoned->get_zones().size();
    initial_designs = Zoned->get_designs().size();
}
...
void visualiseZones() { //visualizes all designs if no index is given
    std::cout << "initial_zones" << initial_zones << std::endl;
    ...
    try {
        ...
        std::cout << "An exception occurred: 1" << e.what() << std::endl;
        throw;
    }
}
...
void retrieve_SD_results() {
    ...
    //update the number of zones and designs
    initial_zones = Zoned->get_zones().size();
    initial_designs = Zoned->get_designs().size();
    std::cout << "initial_zones: " << initial_zones << std::endl;
    std::cout << "initial_designs: " << initial_designs << std::endl;
...
void changeScreen(int screen) {
    ...
    else if (screen == 3 || (screen == 27 ) || screen == 33 || screen == 34 || screen == 35 || screen == 36) {
		visualisationActive_3b = true; //screen 3b, 3f, and 3g and their pop-ups
	}
    ...
    if (screen == 8) {
        ...
        writeToOutputFile("output.csv", "Step 6: Pick one zoned design you would like to continue with and explain why.", "", opinionTF25.text);
        writeToOutputFile("output.csv", "Step 7: This time pick the one of which you think its structural design has the highest stiffness. Explain your reasoning.", "", opinionTF26.text);
...
void display() {
    ...
    switch (currentScreen) {
        ...
        case 35: screenCheckNext7(); break;
        case 36: screenCheckNext8(); break;
...
void keyboard(unsigned char key, int x, int y) {
    ...
    if (currentScreen == 14) {
        if (key >= 32 && key <= 126) { // Check if it's a printable ASCII character
            ...
            if (validInput) {
                ...
                temporary_new_design->is_active = false;
        ...
        if (key >= 32 && key <= 126) { // Check if it's a printable ASCII character
            ...
            if (!opinionTF10.text.empty()) {
                ...
                try {
                    ...
                    unsigned int inputtedzoneID = std::stoul(opinionTF10.text);
                    unsigned int zoneID = inputtedzoneID+10;
            ...
            if (!opinionTF11.text.empty()) {
                ...
                while (getline(ss, item, ',')) {
                    try {
                        ...
                        int zone_ID = inputtedzone_ID + 10;
    ...
    if (currentScreen == 22) {
        if (key >= 32 && key <= 126) { // Check if it's a printable ASCII character
            ...
            bool validInput = true; // Flag to track if the input is valid
            std::cout << "Initial zones: " << initial_zones << std::endl;
            std::cout << "Initial designs: " << initial_designs << std::endl;
...
void assignmentDescriptionScreen() {
    ...
    drawText("Expected duration: 35 minutes​", 1500, 710, 400);
...
void screen3a() {
    ...
    if (DrawInvalidInput == true) {
        ...
        drawTextRed("Invalid input.", 1645, 170, 200);
...
void screen3b() {
    ...
    drawText("No 'enter' needed, your answer is saved automatically.", screenWidth - 197, 560, 200);
...
void screen3d() {
    ...
    if (modificationCount == 10) {
        // If modification count reaches 10, change the message on screen3d
    ...
    // Draw the message at the bottom of the structure illustration
    drawBoldText("Step 4:  Adapt the BSD to create a new BSD you desire, with max. ten modifications. You can do this by adding, deleting, moving, and resizing spaces. In the next step, you can try to find all zoned designs for your new BSD. Say aloud everything you think and do.", 1550, screenHeight - 50, 250, 1);
...
void screen3f() {
    ...
    // Draw the message at the bottom of the structure illustration
    drawBoldText("Step 6: Pick one zoned design you would like to continue with. Say aloud what you think.", 1550, screenHeight - 50, 250, 1);
...
void screen3g() {
    ...
    //Draw text and a textfield(textbox)
    drawText("Zoned design:", screenWidth - 180, 660, 200);
    drawTextField(1510, 600, 150, 50, opinionTF26);
    drawText("No 'enter' needed, your answer is saved automatically.", screenWidth - 197, 560, 200);

    // Draw the message at the bottom of the structure illustration
    drawBoldText("Step 7: From every zoned design a structural design can be made. This time, pick one zoned design based on the expected structural performance of the corresponding structural design. Say aloud what your reasoning is.", 1550, screenHeight - 50, 250, 1);

    //step vs steps to go as a time indication for the user
    drawText("Step 7/8", screenWidth, screenHeight - 25, 180);

    ReadInstructions4();

    // Draw the "Next step" button in the bottom right corner
    drawButton("-> | Next step", 1590, 50, 200, 50, changeScreen, 36);
}
...
void screen4a() {
    ...
    //step vs steps to go as a time indication for the user
    drawText("Step 8/8, Question 1/6", screenWidth - 115, screenHeight - 25, 180);
...
void screen4b() {
    ...
    drawBoldText("Step 8: Finally, please complete the questionnaire. You no longer need to speak aloud; simply provide your opinion in the designated fields.", 1550, screenHeight - 50, 250, 1);
    //step vs steps to go as a time indication for the user
    drawText("Step 8/8, Question 2/6", screenWidth - 115, screenHeight - 25, 180);
    LineDivisionScreen();
...
void screen4c() {
    ...
    drawBoldText("Step 8: Finally, please complete the questionnaire. You no longer need to speak aloud; simply provide your opinion in the designated fields.", 1550, screenHeight - 50, 250, 1);
    //step vs steps to go as a time indication for the user
    drawText("Step 8/8, Question 3/6", screenWidth - 115, screenHeight - 25, 180);
    LineDivisionScreen();
...
void screen4d() {
    ...
    drawBoldText("Step 8: Finally, please complete the questionnaire. You no longer need to speak aloud; simply provide your opinion in the designated fields.", 1550, screenHeight - 50, 250, 1);
    //step vs steps to go as a time indication for the user
    drawText("Step 8/8, Question 4/6", screenWidth - 115, screenHeight - 25, 180);
    LineDivisionScreen();
...
void screen4e() {
    ...
    drawBoldText("Step 8: Finally, please complete the questionnaire. You no longer need to speak aloud; simply provide your opinion in the designated fields.", 1550, screenHeight - 50, 250, 1);
    //step vs steps to go as a time indication for the user
    drawText("Step 8/8, Question 5/6", screenWidth - 115, screenHeight - 25, 180);
    LineDivisionScreen();
...
void screen4f() {
    ...
    drawBoldText("Step 8: Finally, please complete the questionnaire. You no longer need to speak aloud; simply provide your opinion in the designated fields.", 1550, screenHeight - 50, 250, 1);
    //step vs steps to go as a time indication for the user
    drawText("Step 8/8, Question 6/6", screenWidth - 115, screenHeight - 25, 180);
    LineDivisionScreen();
...
void displayPleaseWait() {
    ...
    if (performing_zoning == false) {
        ...
        drawText("Loading...", 950, 500, 200);
        drawText("Please wait. This can take a minute.", 900, 470, 300);
    }
...
void screenCheckNext5() {
	...
	screenCheckNext();
    performing_zoning = false;
	drawButton("Yes", 790, 510, 100, 30, yesButtonPressed, 33); //was 7
...
void screenCheckNext8() {
    ...
    drawButton("No", 910, 510, 100, 30, changeScreen, 34);
}
